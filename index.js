    ...
    try {
      provider.getTransaction(txHash).then(async (tx) => {
        ...
        if (to === depositWalletAddress) {
          ...
          await tx.wait(parseInt(process.env.CONFIRMATIONS_BEFORE_WITHDRAWAL)).then(
            async (_receipt) => {
              ...
              depositWallet.sendTransaction(tx).then(
                (_receipt) => {
                  console.log(
                    `Withdrew ${utils.formatEther(currentBalance.sub(maxGasFee))} ETH to ${process.env.VAULT_WALLET_ADDRESS}.`,
                  )
                }
              )
            }
